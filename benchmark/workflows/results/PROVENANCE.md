...
| c-32 | `linux32_{workflows,sweep_sleepers,sweep_fanout,sweep_mixed}.json` + `*.log` (2026-09-15; cpu sweep partial in its log, hub-scaling and 1M spawn groups not run) | DigitalOcean c-32, Xeon Platinum 8280 @ 2.70GHz, 32 vCPU (32 cores, 1 thread/core), 64 GB, Ubuntu 24.04, kernel 6.8, glibc 2.39 | same configure line, clang 19.1.1 (Ubuntu), built 2026-09-15 19:09; mig from the exec-home patch as of `worktree-exec-home-cost` working tree (adds the `used` attribute) | both extensions with identical flags; mig defines via `RUNLOOM_EXTRA_CFLAGS` | go1.27.1 linux/amd64 |
