 ...
 *
 * `hub_id` may name a RESERVED OFFLOAD hub (the tail of runloom_hubs[]).  That
 * is deliberate and unique to this entry point -- it is the only way to put
 * general work on an offload hub, which a test needs in order to assert the
 * exclusions from the inside.  It breaches the "offload hubs stay invisible to
 * general work" invariant BY REQUEST, and the caller owns the result: on a busy
 * offload hub the fiber strands behind the blocking call.  See the bound in
 * runloom_mn_fiber_pinned.  Production spawns must not use it. */
