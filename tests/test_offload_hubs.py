...
def test_mn_fiber_hub_may_target_an_offload_hub_on_purpose():
    ...
    Every AUTOMATIC placement path bounds itself by runloom_general_hub_count()
    and so cannot name an offload hub even by accident.  mn_fiber(hub=N) can,
    because the caller spelled N out -- and a test needs that to drive the
    exclusions from the inside: offload_fiber() only ever round-robins, so it
    cannot put a CHOSEN fiber on a CHOSEN hub, and nothing else reaches the tail
    of runloom_hubs[] at all.

    Asserted against an IDLE offload hub.  On a busy one the fiber would strand
    behind that hub's blocking call -- which is the documented price of asking,
    not a defect, and is exactly what the test above proves does NOT happen to
    ordinary spawns."""
    K, N_GEN = 2, 4
    seen = {}
...
def test_mn_fiber_hub_still_rejects_a_hub_that_does_not_exist():
    ...
    somewhere else and quietly make every placement assert vacuous."""
    def body():
